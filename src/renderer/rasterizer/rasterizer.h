...
namespace cg::renderer
{
	...
	inline void rasterizer<VB, RT>::draw(size_t num_vertexes, size_t vertex_offset)
	{
		...
		while (vertex_id < vertex_offset + num_vertexes) {
			...
			for(int i = 0; i < 3; ++i) {
				vertices.push_back(vertex_buffer->item(index_buffer->item(vertex_id++)));
			}
			...
			float2 bounding_box_begin = round(clamp(min_vertex, float2{ 0.f, 0.f }, float2{static_cast<float>(width - 1), static_cast<float>(height - 1)}));
