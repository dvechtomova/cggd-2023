...
namespace cg::renderer
{
	...
	{
		for (size_t i{}; i < render_target->get_number_of_elements(); i++) {
	...
	{
		for (size_t i{}; i < index_buffers.size(); ++i) {
			auto& index_buffer = index_buffers[i];
			auto& vertex_buffer = vertex_buffers[i];
			aabb<VB> aabb;
			for(size_t index{}; index < index_buffer->get_number_of_elements();) {
...
#pragma omp parallel for
	...
	float2 raytracer<VB, RT>::get_jitter(int frame_id)
	{
		...
		auto lam = [&](int base) {
			...
			while (index > 0) {
				if (base == base_x) {
					result.x += static_cast<float>(index % base) * fraction;
				} else {
					result.y += static_cast<float>(index % base) * fraction;
				}
				index /= base;
				fraction *= inv_base;
			}
